...
public class HarnessCodeGenericApiErrorHandler implements ScmApiErrorHandler {
  ...
  public void handleError(int statusCode, String errorMessage, ErrorMetadata errorMetadata) throws WingsException {
    switch (statusCode) {
            ...
            case 500 -> throw new ScmBadRequestException(ErrorMessageFormatter.formatMessage("not found",
                    errorMetadata));
            default -> {
                log.error(String.format("New error in harness code ops. Status code: [%d]. message: [%s]",
                        statusCode, errorMessage), new Exception());
