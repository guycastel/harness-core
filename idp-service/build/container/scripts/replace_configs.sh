...
if [[ "" != "$DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_TOPIC_NAME" ]]; then
  export DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_TOPIC_NAME; yq -i '.debeziumConsumerConfigs.backstageCatalog.topic=env(DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_TOPIC_NAME)' $CONFIG_FILE
fi

if [[ "" != "$DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_BATCH_SIZE" ]]; then
  export DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_BATCH_SIZE; yq -i '.debeziumConsumerConfigs.backstageCatalog.batchSize=env(DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_BATCH_SIZE)' $CONFIG_FILE
fi

if [[ "" != "$DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_MAX_PROCESSING_TIME_SECONDS" ]]; then
  export DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_MAX_PROCESSING_TIME_SECONDS; yq -i '.debeziumConsumerConfigs.backstageCatalog.maxProcessingTimeSeconds=env(DEBEZIUM_BACKSTAGE_CATALOG_CONSUMER_MAX_PROCESSING_TIME_SECONDS)' $CONFIG_FILE
fi

if [[ "" != "$DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_TOPIC_NAME" ]]; then
  export DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_TOPIC_NAME; yq -i '.debeziumConsumerConfigs.backstageScaffolderTasks.topic=env(DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_TOPIC_NAME)' $CONFIG_FILE
fi

if [[ "" != "$DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_BATCH_SIZE" ]]; then
  export DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_BATCH_SIZE; yq -i '.debeziumConsumerConfigs.backstageScaffolderTasks.batchSize=env(DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_BATCH_SIZE)' $CONFIG_FILE
fi

if [[ "" != "$DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_MAX_PROCESSING_TIME_SECONDS" ]]; then
  export DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_MAX_PROCESSING_TIME_SECONDS; yq -i '.debeziumConsumerConfigs.backstageScaffolderTasks.maxProcessingTimeSeconds=env(DEBEZIUM_BACKSTAGE_SCAFFOLDER_TASKS_CONSUMER_MAX_PROCESSING_TIME_SECONDS)' $CONFIG_FILE
fi
