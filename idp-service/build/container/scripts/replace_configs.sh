...
if [[ "" != "$SCORECARDS_REDIS_EVENT_CONSUMER_THREADS" ]]; then
  export SCORECARDS_REDIS_EVENT_CONSUMER_THREADS; yq -i '.numberOfThreadsToUseForConsumers.SCORECARDS_REDIS_EVENT_CONSUMER_THREADS=env(SCORECARDS_REDIS_EVENT_CONSUMER_THREADS)' $CONFIG_FILE
fi
